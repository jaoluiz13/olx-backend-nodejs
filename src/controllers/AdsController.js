...
module.exports = {
  ad: async (req, res) => {
    ...
    if (!title || !cat) {
      res.json({ erro: "titulo ou categoria não foram preenchidos" });
      return;
    }
    if (price) {
    ...
    newAd.title = title;
    newAd.category = cat;
    newAd.priceNegotiable = price == "true" ? true : false;
    ...
    if (req.file && req.files.img) {
      if (req.files.img.length == undefined) {
        if (
          ["image/jpeg", "imagejpg", "image/png"].includes(
  ...
  editAction: async (req, res) => {
      ...
      if(!ad){
          res.json({error:'Anuncio nao encontrado'});
          return;
      }
      const user = await User.findOne({token}).exec();
      if(user._id.toString()!= ad.idUser){
          res.json({erro:'Você só pode alterar anuncios que pertecem a você'});
          return;
      }
     ...
     if(cat){
       ...
       if(!category){
           res.json({erro:'Categoria Informada não existe'});
       }
